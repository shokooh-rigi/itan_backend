...
@register.simple_tag
def get_primary_heat_exchanger_design_value(request, design_field, equipment):
    if request.method == 'POST':
        return request.POST.get(f'design_value_{design_field.id}')
    if PrimaryHeatExchangerEquipment.objects.get(id=equipment.id).equipment:
        ...
        return_value = PrimaryHeatExchangerSheetData.objects.filter(data_type=1, primary_heat_exchanger_equipment=equipment, sheet_field=design_field)
...
@register.simple_tag
def get_primary_heat_exchanger_actual_value(request, actual_field, equipment):
    if request.method == 'POST':
        return request.POST.get(f'actual_value_{actual_field.id}')
    return_value = PrimaryHeatExchangerSheetData.objects.filter(data_type=2, primary_heat_exchanger_equipment=equipment, sheet_field=actual_field)
    if return_value.count() > 0:
        return return_value.first().value
    else:
