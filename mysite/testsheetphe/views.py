...
from .models import PHEEquipment, PHESheetData
        ...
        if form.is_valid():
            if request.POST.get("next"):
                ...
                for n in range(form.cleaned_data['equipment_quantity']):
                    primary_heat_exchanger_equipment = PHEEquipment(sheet=sheet, equipment_type=equipment_type)
...
def primary_heat_exchanger_sheet_equipment_list(request, sheet_id):
    ...
    primary_heat_exchanger_equipments = PHEEquipment.objects.filter(sheet=my_sheet).order_by('id')
    if project_name:
...
def primary_heat_exchanger_equipment_add(request, sheet_id):
    my_sheet = DataSheet.objects.get(id=sheet_id)
    primary_heat_exchanger_equipment = PHEEquipment(sheet=my_sheet)
    primary_heat_exchanger_equipment.save()
...
def get_pdf_parameters(sheet_id, is_report_pdf: bool):
    total_pdf_row = 4
    my_sheet = DataSheet.objects.get(id=sheet_id)
    primary_heat_exchanger_equipments = PHEEquipment.objects.filter(sheet=my_sheet, design_data_entry_completed=True).order_by('id')
    if is_report_pdf:
    ...
    for primary_heat_exchanger_equipment in primary_heat_exchanger_equipments:
        ...
        if is_report_pdf:
            primary_heat_exchanger_sheetdatas = primary_heat_exchanger_equipment.phesheetdata_set.all()
        else:
            primary_heat_exchanger_sheetdatas = primary_heat_exchanger_equipment.phesheetdata_set.filter(data_type=1)
    ...
    return {
        ...
        'file_name': 'Primary Heat Exchanger Sheet {}-{}{}'.format(my_sheet.project.proposal.estimate.project.name,
...
def primary_heat_exchanger_common_data(request, primary_heat_exchanger_equipment_id):
    primary_heat_exchanger_equipment = get_object_or_404(PHEEquipment, id=primary_heat_exchanger_equipment_id)
    this_equipment = None
    if primary_heat_exchanger_equipment.equipment:
    ...
    if request.method == 'POST':
        ...
        if request.POST.get("next"):
            ...
            for actual_field in actual_fields:
                ...
                num_results = PHESheetData.objects.filter(data_type=2, primary_heat_exchanger_equipment=primary_heat_exchanger_equipment,
                                                           sheet_field=actual_field).count()

                if num_results > 0:
                    PHESheetData.objects.filter(data_type=2, primary_heat_exchanger_equipment=primary_heat_exchanger_equipment,
                                                 sheet_field=actual_field).update(value=new_value)
                else:
                    new_object = PHESheetData(data_type=2, primary_heat_exchanger_equipment=primary_heat_exchanger_equipment,
