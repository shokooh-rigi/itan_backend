...
@register.simple_tag
def get_chiller_design_value(request, design_field, equipment):
    if request.method == 'POST':
        return request.POST.get(f'design_value_{design_field.id}')
    if ChillerEquipment.objects.get(id=equipment.id).equipment:
        ...
        return_value = ChillerSheetData.objects.filter(data_type=1, chiller_equipment=equipment, sheet_field=design_field)
...
@register.simple_tag
def get_chiller_actual_value(request, actual_field, equipment):
    if request.method == 'POST':
        return request.POST.get(f'actual_value_{actual_field.id}')
    return_value = ChillerSheetData.objects.filter(data_type=2, chiller_equipment=equipment, sheet_field=actual_field)
    if return_value.count() > 0:
        return return_value.first().value
    else:
