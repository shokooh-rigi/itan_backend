...
class InductionUnitEquipment(models.Model):
...
class InductionUnitSheetData(models.Model):
    data_type = models.PositiveSmallIntegerField(choices=DataTypeChoices.get_items(), default=1, null=False)
    induction_unit_equipment = models.ForeignKey(InductionUnitEquipment, on_delete=models.CASCADE, blank=False, null=False)
    ...
    def __str__(self):
        return str(self.sheet_field) + ' ' + str(self.induction_unit_equipment)
