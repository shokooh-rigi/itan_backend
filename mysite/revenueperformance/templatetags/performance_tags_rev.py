...
def total_quoted_count(customer, from_date_obj, to_date_obj):
    quoted_count = Proposal.objects.filter(estimate__customer__company=customer, created_on__range=(from_date_obj, to_date_obj)).count()
    return quoted_count
...
def total_quoted_price(customer, from_date_obj, to_date_obj):
    total_quote = 0
    quote_list = Proposal.objects.filter(estimate__customer__company=customer, created_on__range=(from_date_obj, to_date_obj))
