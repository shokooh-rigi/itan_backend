...
class VbsEquipment(models.Model):
...
class VbsSheetData(models.Model):
    data_type = models.PositiveSmallIntegerField(choices=DataTypeChoices.get_items(), default=1, null=False)
    vbs_equipment = models.ForeignKey(VbsEquipment, on_delete=models.CASCADE, blank=False, null=False)
    ...
    def __str__(self):
        return str(self.sheet_field) + ' ' + str(self.vbs_equipment)
