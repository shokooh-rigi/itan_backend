...
from .models import VbsEquipment, VbsSheetData


# Create your views here.
SHEET_TYPE_NAME = 'V.A.V. Box Schedule'
...
def sheet_list(request):
    ...
    parameters = {
        ...
        'generate_test_sheet_url': reverse('vbsSheetAdd'),
        'equipment_add_url': '/vbs/equipment_add/',
        'equipment_list_url': '/vbs/equipments_list/',
        'sheet_delete_url': '/vbs/delete/',
        'sheets': sheets,
        'WEB_URL': WEB_URL,
        'MEDIA_URL': MEDIA_URL,
    }
...
def sheet_add(request):
    form = VbsSheetForm(request.POST or None, request.FILES or None)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if request.POST.get("next"):
                ...
                sheet.save()
                equipment_type = None
                if Equipment.objects.filter(name='VARIABLE AIR VOLUME VAV').count() > 0:
                    equipment_type = Equipment.objects.filter(name='VARIABLE AIR VOLUME VAV').first()
                for n in range(form.cleaned_data['equipment_quantity']):
                    vbs_equipment = VbsEquipment(sheet=sheet, equipment_type=equipment_type)
                    vbs_equipment.save()
                return redirect('vbsSheetEquipmentList', sheet.id)
    parameters = {
        'sheet_type_name': SHEET_TYPE_NAME,
        'form': form,
        'orders': orders,
...
def sheet_equipment_list(request, sheet_id):
    ...
    vbs_equipments = VbsEquipment.objects.filter(sheet=my_sheet).order_by('id')
    if project_name:
        vbs_equipments = vbs_equipments.filter(Q(unit_number__icontains=project_name) | Q(model_number__icontains=project_name)).distinct()
    parameters = {
        'sheet_type_name': SHEET_TYPE_NAME,
        'equipments': vbs_equipments,
        'my_sheet': my_sheet,
        'sheet_id': sheet_id,
        'WEB_URL': WEB_URL,
        'MEDIA_URL': MEDIA_URL,
    }
    return render(request, "vbsSheetEquipmentsList.html", parameters)


@login_required
def equipment_add(request, sheet_id):
    my_sheet = DataSheet.objects.get(id=sheet_id)
    vbs_equipment = VbsEquipment(sheet=my_sheet)
    vbs_equipment.save()
    return redirect('vbsSheetEquipmentList', sheet_id)


def get_pdf_parameters(sheet_id, is_report_pdf: bool):
    total_pdf_row = 22
    my_sheet = DataSheet.objects.get(id=sheet_id)
    vbs_equipments = VbsEquipment.objects.filter(sheet=my_sheet, design_data_entry_completed=True).order_by('id')
    if is_report_pdf:
        vbs_equipments = vbs_equipments.filter(actual_data_entry_completed=True)
    total_pages = math.ceil(vbs_equipments.count() / total_pdf_row)
    vbs_equipment_data = []
    vbs_equipment_page = []
    i = 0
    for vbs_equipment in vbs_equipments:
        vbs_equipment_obj = {}
        if is_report_pdf:
            vbssheetdatas = vbs_equipment.vbssheetdata_set.all()
        else:
            vbssheetdatas = vbs_equipment.vbssheetdata_set.filter(data_type=1)
        for vbs_data in vbssheetdatas:
            vbs_equipment_obj[vbs_data.sheet_field.field_name + '-' + str(vbs_data.data_type)] = vbs_data.value
        vbs_equipment_page.append(vbs_equipment_obj)
        i += 1
        if i == total_pdf_row:
            vbs_equipment_data.append(vbs_equipment_page)
            vbs_equipment_page = []
            i = 0
    if vbs_equipment_page:
        vbs_equipment_data.append(vbs_equipment_page)
    ...
    print(vbs_equipment_data)

    return {
        'total_pdf_row': total_pdf_row,
        'empty_row_range': range(total_pdf_row - (vbs_equipments.count() % total_pdf_row)),
        'form': {
            'total_pages_range': range(total_pages),
            'total_pages': total_pages,
            'my_sheet': my_sheet,
            'vbs_equipments': vbs_equipments,
            'vbs_equipment_data': vbs_equipment_data
        },
        'sheet_type_name': SHEET_TYPE_NAME,
...
def equipments_generate_tech_pdf(request, sheet_id):
    ...
    pdf_name, pdf_path = PDFRender.render_to_file('pdfTemplates/vbsSheetPDFTemplate.html', parameters,
                                                  'vbsPDFReport')
...
@login_required
def common_data(request, equipment_id):
    equipment = get_object_or_404(VbsEquipment, id=equipment_id)
    this_equipment = None
    if equipment.equipment:
        this_equipment = equipment.equipment
    form = VbsSheetEquipmentForm(request.POST or None, request.FILES or None, instance=equipment)
    print(equipment.equipment_type)
    manufacturers = EquipmentManufacturer.objects.filter(equipmentdb__equipment_type=equipment.equipment_type).distinct()
    equipment_db = EquipmentDb.objects.filter(equipment_type=equipment.equipment_type)

    if request.method == 'POST':
        if request.POST.get("cancel"):
            return redirect('vbsSheetEquipmentList', equipment.sheet.id)
        if form.is_valid():
            if request.POST.get("save"):
                form.cleaned_data['sheet'] = equipment.sheet
    ...
    return render(request, "vbsSheetEquipmentGeneralData.html", parameters)


@login_required
def design_data(request, equipment_id):
    vbs_equipment = get_object_or_404(VbsEquipment, id=equipment_id)

    design_fields = TestSheetField.objects.filter(show_in_design=True, test_sheet__name__iexact=SHEET_TYPE_NAME)

    if request.method == 'POST':
        if request.POST.get("cancel"):
            return redirect('vbsSheetEquipmentList', vbs_equipment.sheet.id)
        if request.POST.get("next"):

            for design_field in design_fields:
                new_value = request.POST.get(f'supply_design_value_{design_field.id}').strip()

                num_results = VbsSheetData.objects.filter(data_type=1, vbs_equipment=vbs_equipment,
                                                           sheet_field=design_field).count()

                if num_results > 0:
                    VbsSheetData.objects.filter(data_type=1, vbs_equipment=vbs_equipment,
                                                 sheet_field=design_field).update(value=new_value)
                else:
                    new_object = VbsSheetData(data_type=1, vbs_equipment=vbs_equipment,
                                               sheet_field=design_field,
                                               value=new_value)
                    new_object.save()

            vbs_equipment.design_data_entry_completed = True
            vbs_equipment.save()
            return redirect('vbsSheetEquipmentList', vbs_equipment.sheet.id)

    parameters = {
        'sheet_type_name': SHEET_TYPE_NAME,
        'vbs_equipment': vbs_equipment,
        'design_fields': design_fields,
    }
    return render(request, "vbsSheetEquipmentDesignData.html", parameters)
...
def actual_data(request, equipment_id):
    ...
    if request.method == 'POST':
        ...
        if request.POST.get("next"):
            ...
            for actual_field in actual_fields:
                ...
                num_results = VbsSheetData.objects.filter(data_type=2, vbs_equipment=vbs_equipment,
                                                           sheet_field=actual_field).count()

                if num_results > 0:
                    VbsSheetData.objects.filter(data_type=2, vbs_equipment=vbs_equipment,
                                                 sheet_field=actual_field).update(value=new_value)
                else:
                    new_object = VbsSheetData(data_type=2, vbs_equipment=vbs_equipment,
                                               sheet_field=actual_field,
                                               value=new_value)
                    new_object.save()

            vbs_equipment.actual_data_entry_completed = True
            vbs_equipment.save()
            return redirect('vbsSheetEquipmentList', vbs_equipment.sheet.id)
    ...
    return render(request, "vbsSheetEquipmentActualData.html", parameters)
...
def sheet_delete(request, sheet_id):
    ...
    return render(request, "vbsSheetDelete.html", parameters)
