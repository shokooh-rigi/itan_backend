...
from ..scheduler.models import Maintenance, Schedule, ScheduleTech
...
from ..order.templatetags.order_tags import (
    order_tech_final_price_calculator,
    order_tech_predemo_price_calculator,
)
...
def schedule_list(request):
    ...
    schedule_start_date = datetime.datetime.strptime(
        schedule_start_date, "%m/%d/%Y"
    ).date()
    if request.user.profile.status == 1:
        maintenance_list = Maintenance.objects.filter(
            assigned_to_employee=request.user
        ).filter(schedule_start__gte=schedule_start_date)
        scheduled = Schedule.objects.filter(
            scheduletech__assigned_to_employee=request.user
        ).filter(schedule_start__gte=schedule_start_date)
    elif request.user.profile.status == 2:
        maintenance_list = Maintenance.objects.filter(
            assigned_to_contractor=request.user
        ).filter(schedule_start__gte=schedule_start_date)
        scheduled = Schedule.objects.filter(
            scheduletech__assigned_to_contractor=request.user
        ).filter(schedule_start__gte=schedule_start_date)
    else:
    ...
    for schedule in scheduled:
        if request.user.profile.status == 1:
            this_schedule_tech = ScheduleTech.objects.get(
                schedule=schedule, assigned_to_employee=request.user
            )
        elif request.user.profile.status == 2:
            this_schedule_tech = ScheduleTech.objects.get(
                schedule=schedule, assigned_to_contractor=request.user
            )
        else:
        ...
        if schedule.order.proposal.estimate.project.address_line_2:
            full_address += (
                " " + schedule.order.proposal.estimate.project.address_line_2
            )
            ...
            if schedule_tech.assigned_to_employee:
                ...
                if schedule_tech.assigned_to_employee.last_name:
                    assigned_to_employees_names.append(
                        schedule_tech.assigned_to_employee.first_name
                        + " "
                        + schedule_tech.assigned_to_employee.last_name
                    )
                else:
                    assigned_to_employees_names.append(
                        schedule_tech.assigned_to_employee.email
                    )
                ...
                if schedule_tech.assigned_to_contractor.last_name:
                    assigned_to_contractors_names.append(
                        schedule_tech.assigned_to_contractor.first_name
                        + " "
                        + schedule_tech.assigned_to_contractor.last_name
                    )
                else:
                    assigned_to_contractors_names.append(
                        schedule_tech.assigned_to_contractor.email
                    )
        any_assigned = False
        ...
        try:
            ...
            if schedule.order.techlabel.point_of_contact_office_phone:
                poc_office_phone = (
                    schedule.order.techlabel.point_of_contact_office_phone
                )
        ...
        response_data.append(
            {
                ...
                "price": (
                    ...
                    else order_tech_final_price_calculator(
                        schedule.order.proposal.estimate.id, schedule.order
                    )
                ),
    ...
    for maintenance in maintenance_list:
        ...
        if maintenance.order:
            if maintenance.order.proposal.estimate.project.address_line_1:
                full_address += (
                    maintenance.order.proposal.estimate.project.address_line_1
                )
            if maintenance.order.proposal.estimate.project.address_line_2:
                full_address += (
                    " " + maintenance.order.proposal.estimate.project.address_line_2
                )
        ...
        if maintenance.assigned_to_employee:
            ...
            if maintenance.assigned_to_employee.last_name:
                assigned_to_employees_names.append(
                    maintenance.assigned_to_employee.first_name
                    + " "
                    + maintenance.assigned_to_employee.last_name
                )
            else:
                assigned_to_employees_names.append(
                    maintenance.assigned_to_employee.email
                )
            ...
            if maintenance.assigned_to_contractor.last_name:
                assigned_to_contractors_names.append(
                    maintenance.assigned_to_contractor.first_name
                    + " "
                    + maintenance.assigned_to_contractor.last_name
                )
            else:
                assigned_to_contractors_names.append(
                    maintenance.assigned_to_contractor.email
                )
        any_assigned = False
        ...
        if maintenance.order:
            if maintenance.maintenance_type == 1:
                maintenance_title = "Maintenance " + str(
                    maintenance.order.project_number
                    + "<br />"
                    + str(maintenance.order.proposal.estimate.project)
                )
            else:
                maintenance_title = "Lost Time " + str(
                    maintenance.order.project_number
                    + "<br />"
                    + str(maintenance.order.proposal.estimate.project)
                )
        ...
        try:
            ...
            if maintenance.order.techlabel.point_of_contact_office_phone:
                poc_office_phone = (
                    maintenance.order.techlabel.point_of_contact_office_phone
                )
        ...
        test_sheet_count = 0
        if maintenance.order:
            test_sheet_count = Sheet.objects.filter(
                project__id=maintenance.order.id
            ).count()
        if test_sheet_count > 0:
        ...
        response_data.append(
            {
                ...
                "assigned_to_contractors_names": assigned_to_contractors_names,
                "project_number": (
                    maintenance.order.project_number if maintenance.order else 0
                ),
                "project_name": (
                    str(maintenance.order.proposal.estimate.project)
                    if maintenance.order
                ...
                "customer": (
                    str(maintenance.order.proposal.estimate.customer.company.name)
                    if maintenance.order
                    else ""
                ),
                "engineer": (
                    str(maintenance.order.proposal.estimate.engineer.company.name)
                    if maintenance.order
                    else ""
                ),
                "predemo": (
                    maintenance.order.proposal.estimate.estimatedetails.pre_demo
                    if maintenance.order
                    else 0
                ),
                "poc_name": poc_name,
                ...
                "poc_office_phone": poc_office_phone,
                "control_system": (
                    str(maintenance.order.control_system) if maintenance.order else ""
                ),
                "special_instruction": special_instruction,
...
def update_note(request):
    if request.method == "POST":
        if request.POST.get("schedule_id"):
            ...
            if request.user.profile.status == 1:
                schedule_update = get_object_or_404(
                    ScheduleTech,
            ...
            return JsonResponse(
                {"result": True, "msg": "Note updated on database successfully."}
            )
        elif request.POST.get("maintenance_id"):
            maintenance_id = request.POST.get("maintenance_id")
            note = request.POST.get("note_text")
            if request.user.profile.status == 1:
                maintenance_update = get_object_or_404(
                    Maintenance, id=maintenance_id, assigned_to_employee=request.user
                )
            else:
                maintenance_update = get_object_or_404(
                    Maintenance, id=maintenance_id, assigned_to_contractor=request.user
                )
    ...
    if request.method == "POST":
        if request.POST.get("schedule_id"):
            ...
            temp_path = os.path.join(
                os.path.abspath(os.path.dirname("__file__")), "media/uploads/techfiles"
            )
            ...
            if request.user.last_name:
                zip_file_name = (
                    str(schedule_update.schedule.id)
                    + ". "
            ...
            if schedule_update.tech_upload:
                ...
                response = requests.get(
                    s3.get_bucket_object(
                        "media/" + str(schedule_update.tech_upload.file)
                    )
                )
                ...
                s3.delete_file_from_bucket(
                    key=settings.MEDIA_URL + str(schedule_update.tech_upload)
                )
            ...
            return JsonResponse({"result": True, "msg": "Upload successful!"})
        elif request.POST.get("maintenance_id"):
            maintenance_id = request.POST.get("maintenance_id")
            if request.user.profile.status == 1:
                maintenance_update = get_object_or_404(
            ...
            if maintenance_update.tech_upload:
                ...
                create_zip_file(files, temp_path, zip_file_name)
                maintenance_update.tech_upload = (
                    settings.UPLOAD_URL + "techfiles/" + zip_file_name
                )
                maintenance_update.save()
