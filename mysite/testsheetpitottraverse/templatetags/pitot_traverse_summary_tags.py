...
def get_pitot_traverse_summary_design_value(request, design_field, equipment):
    ...
    return_value = PitotTraverseSheetData.objects.filter(pitot_traverse_summary_equipment=equipment, sheet_field=design_field, data_type=1)
...
def get_pitot_traverse_summary_actual_value(request, actual_field, equipment):
    ...
    return_value = PitotTraverseSheetData.objects.filter(pitot_traverse_summary_equipment=equipment, sheet_field=actual_field, data_type=2)
    if return_value.count() > 0:
        return return_value.first().value
    else:
