...
def projects_list(request):
    ...
    object_list = TechUploadOrderForm.objects.filter(customer__name__icontains=search).order_by(ordering)
...
def documents_add(request):
    form = TechUploadOrderForm(request.POST or None, request.FILES or None, initial={'created_by': request.user})
...
@login_required
def techupload_edit(request, project_id):
