...
from mysite.core.models import LicenseInfo, LicenseFiles, ModulesToEmailTemplateRelation
from mysite.order.models import Order
from .forms import SettlementForm, SettledScheduleForm
from .models import Settlement, SettledSchedule, SettledMaintenances
...
def settlement_list(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            if ModulesToEmailTemplateRelation.objects.filter(module=8).exists():
                body_content = get_object_or_404(
                    ModulesToEmailTemplateRelation, module=8
                ).template.content
            else:
                body_content = "There was no email template defined for 'Settlement'."
            body_content = htmlbodytemplate_tag_converter(
                1, body_content, request, customer
            )
            if ModulesToEmailTemplateRelation.objects.filter(module=5).exists():
                footer_content = ModulesToEmailTemplateRelation.objects.get(
                    module=5
                ).template.content
            else:
                footer_content = (
                    "There was no email template defined for 'Email Footer'."
                )
            footer_content = htmlbodytemplate_tag_converter(
                1, footer_content, request, customer
            )
            ...
            try:
                ...
                msg.attach_file(
                    "media/pdfs/settlement/settlement-" + str(email_id) + ".pdf"
                )
                msg.send()
    ...
    object_list = Settlement.objects.filter(
...
def settlement_orders(request, settlement_id):
    ...
    form = SettledScheduleForm(
        request.POST or None,
        request.FILES or None,
        initial={"settlement": this_settlement.id},
    )
    schedules = (
        Schedule.objects.filter(
            scheduletech__assigned_to_contractor=this_settlement.contractor,
            scheduletech__settlement=False,
        )
        .filter(
            schedule_start__gt=this_settlement.settlement_start,
            schedule_end__lt=this_settlement.settlement_end,
        )
        .distinct()
    ...
    maintenances = (
        Maintenance.objects.filter(
            assigned_to_contractor=this_settlement.contractor, settlement=False
        )
        .filter(
            schedule_start__gt=this_settlement.settlement_start,
            schedule_end__lt=this_settlement.settlement_end,
        )
        .filter(Q(maintenance_type=1) | Q(maintenance_type=2))
        .order_by("-created_on")
        .distinct()
        .order_by("schedule_start")
    )
        ...
        if request.POST.get("next"):
            for schedule in schedules:
                ...
                if request.POST.get("order-include-" + str(schedule.id)):
                    ...
                    if request.POST.get("or-toggle-" + str(schedule.id)):
                        order_schedules = Schedule.objects.filter(
                            order=schedule.order, pre_demo=schedule.pre_demo
                        )
                        order_total_scheduled = 0
                        for order_schedule in order_schedules:
                            order_total_scheduled += (
                                (
                                    order_schedule.schedule_end
                                    - order_schedule.schedule_start
                                ).total_seconds()
                            ) / 3600
                        settled_type = True
                        ...
                        schedule_duration_in_hours = (
                            (
                                schedule.schedule_end - schedule.schedule_start
                            ).total_seconds()
                        ) / 3600
                        total_schedules_duration = (
                            total_schedules_duration + schedule_duration_in_hours
                        )

                        completion_percentage = round(
                            total_schedules_duration / order_total_scheduled * 100
                        )

                        if request.POST.get("quoted-price-" + str(schedule.id)):
                            override_quoted_price = float(
                                request.POST.get("quoted-price-" + str(schedule.id))
                            )
                            quoted_price = override_quoted_price
                        else:
                            if (
                                schedule.order.proposal.estimate.estimatedetails.pre_demo
                                > 0
                            ):
                                if schedule.pre_demo:
                                    ...
                                    predemo_offset = float(
                                        schedule.order.predemo_offset
                                    )
                                    ...
                                    quoted_price = (
                                        order_price - pre_demo_price - final_offset
                                    )
                            else:
                                quoted_price = float(
                                    order_total_calculator(
                                        schedule.order.proposal.estimate.id,
                                        schedule.order,
                                    )
                                )
                        ...
                        settle_value = (
                            (
                                float(
                                    this_settlement.contractor.profile.interest_percentage
                                )
                                / 100
                                * quoted_price
                            )
                            * schedule_tech.involvement_percentage
                            / 100
                            * completion_percentage
                            / 100
                        )
                        total_settled_value = total_settled_value + round(
                            settle_value, 2
                        )
                        if prev_payment:
                            total_settled_value = (
                                (quoted_price - float(prev_payment))
                                * (
                                    float(
                                        this_settlement.contractor.profile.interest_percentage
                                    )
                        ...
                        if request.POST.get("total-hour-" + str(schedule.id)):
                            override_total_duration = float(
                                request.POST.get("total-hour-" + str(schedule.id))
                            )
                        ...
                        if (
                            ...
                            quoted_price = quoted_price - offset
                        settle_value = float(
                            this_settlement.contractor.profile.hourly_rate
                        ) * float(schedule_duration_in_hours)
                        total_settled_value = total_settled_value + round(
                            settle_value, 2
                        )
                        if prev_payment:
                    ...
                    if request.POST.get("settle-override-" + str(schedule.id)):
                        settled_schedule = SettledSchedule(
                            settlement=this_settlement,
                            schedule=schedule,
                            ...
                            settle_override=float(
                                request.POST.get("settle-override-" + str(schedule.id))
                            ),
                        )
                    else:
                        settled_schedule = SettledSchedule(
            ...
            for maintenance in maintenances:
                ...
                if request.POST.get("maintenance-include-" + str(maintenance.id)):
                    if request.POST.get(
                        "maintenance-total-hour-" + str(maintenance.id)
                    ):
                        override_total_duration = float(
                            request.POST.get(
                                "maintenance-total-hour-" + str(maintenance.id)
                        ...
                        schedule_duration_in_hours = (
                            (
                                maintenance.schedule_end - maintenance.schedule_start
                            ).total_seconds()
                        ) / 3600
                    total_schedules_duration = schedule_duration_in_hours
                    settle_value = float(
                    ...
                    maintenance.save()
                settled_maintenance = SettledMaintenances(
                    settlement=this_settlement,
...
def settlement_edit(request, settlement_id):
    ...
    schedules = SettledSchedule.objects.filter(settlement_id=settlement_id).order_by(
        "schedule__schedule_start"
    )
    maintenances = SettledMaintenances.objects.filter(
        settlement_id=settlement_id
    ).order_by("maintenance___schedule_start")
    if request.method == "POST":
        if request.POST.get("cancel"):
            return redirect("settlementHome")
        if request.POST.get("next"):
            for schedule in schedules:
                ...
                if request.POST.get("or-toggle-" + str(schedule.id)):
                    ...
                    order_schedules = Schedule.objects.filter(
                        ...
                        pre_demo=schedule.schedule.pre_demo,
                    )
                    order_total_scheduled = 0
                    for order_schedule in order_schedules:
                        order_total_scheduled += (
                    ...
                    schedule_duration_in_hours = (
                        (
                            ...
                            - schedule.schedule.schedule_start
                        ).total_seconds()
                    ) / 3600
                    total_schedules_duration = (
                        total_schedules_duration + schedule_duration_in_hours
                    ...
                    settle_value = float(
                        this_settlement.contractor.profile.hourly_rate
                    ) * float(total_schedules_duration)
                    total_settled_value = total_settled_value + round(settle_value, 2)
                    if prev_payment:
                        total_settled_value = (
                            (quoted_price - float(prev_payment))
            ...
            for maintenance in maintenances:
                override_total_duration = float(
                    request.POST.get("maintenance-total-hour-" + str(maintenance.id))
                )
                total_schedules_duration = override_total_duration
                settle_value = float(
                    this_settlement.contractor.profile.hourly_rate
...
def settlement_view(request, settlement_id):
    ...
    settled_schedules = SettledSchedule.objects.filter(settlement=settlement).order_by(
        "schedule__schedule_start"
    )
    ...
    parameters = {
        "file_name": "settlement-" + str(settlement_id),
...
def generate_pdf(request, settlement_id):
    ...
    parameters = {
        "file_name": str(settlement.contractor.first_name)
        + " "
        + str(settlement.settlement_start.strftime("%B %Y")),
        "settlement": settlement,
        "settled_schedules": settled_schedules,
        "settled_maintenances": settled_maintenances,
        "datetime": datetime.datetime.now(),
    ...
    pdf_name, pdf_path = PDFRender.render_to_file(
        "pdfTemplates/settlementTemplate.html", parameters, "settlementPDF"
    )
