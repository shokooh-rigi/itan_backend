...
class DataSheetForm(forms.Form):
    ...
    selectedItem = forms.ChoiceField(
        choices=ITEM_CHOICES, widget=forms.Select(attrs={"class": "form-control"})
    )
...
class ProposalWidget(s2forms.ModelSelect2Widget):
    search_fields = ["id", "estimate__project__name__icontains"]
...
class OrderForm(ModelForm):
    equipment_submittal = forms.FileField(
        required=False,
        widget=forms.ClearableFileInput(attrs={"allow_multiple_selected": True}),
    )
    site_pictures = forms.FileField(
        required=False,
        widget=forms.ClearableFileInput(attrs={"allow_multiple_selected": True}),
    )
    test_sheets = forms.FileField(
        required=False,
        widget=forms.ClearableFileInput(attrs={"allow_multiple_selected": True}),
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["architect_name"].queryset = Person.objects.filter(
            company__company_type__name__iexact="architect"
        )
...
class GeneralNoteForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "general_notes_and_comments": forms.Textarea(
                attrs={
