...
from mysite.estimator.templatetags.estimator_tags import (
    estimate_total_calculator,
    estimate_predemo_calculator,
    estimate_sub_total_dalt_calculator,
)
from ..models import ChangeOrder, ChangeOrderService
...
def calculate_total_amount_due(invoice):
    if invoice.invoice_type == 1:
        sub_total = order_total_calculator(
            invoice.order.proposal.estimate.id, invoice.order
        )
    elif invoice.invoice_type == 2:
        sub_total = order_predemo_calculator(
            invoice.order.proposal.estimate.id, invoice.order
        )
    elif invoice.invoice_type == 4:
        sub_total = order_dalt_calculator(
            invoice.order.proposal.estimate.id, invoice.order
        )
    else:
        sub_total = order_final_calculator(
            invoice.order.proposal.estimate.id, invoice.order
        )
