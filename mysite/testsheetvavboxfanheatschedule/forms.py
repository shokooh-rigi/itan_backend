...
class VbfhsSheetForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super(VbfhsSheetForm, self).__init__(*args, **kwargs)
...
class VbfhsSheetEquipmentForm(ModelForm):
    class Meta:
        model = VbfhsEquipment
        fields = [
            'sheet',
        ]

    def __init__(self, *args, **kwargs):
        super(VbfhsSheetEquipmentForm, self).__init__(*args, **kwargs)
        for visible in self.visible_fields():
            visible.field.widget.attrs['class'] = 'form-control'
        for field in self.fields.values():
            field.error_messages = {'required': '{fieldname} field is required'.format(fieldname=field.label)}
