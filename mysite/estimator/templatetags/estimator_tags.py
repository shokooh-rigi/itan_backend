...
def pdf_filename_generator(estimate_id, pdf_type, version=None):
    ...
    estimate_filename = (
        pdf_type + longidname + "_" + estimate.project.name + estimate_version
    )
    return (
        estimate_filename.replace(" ", "_")
        .replace("!", "")
        .replace("@", "")
        .replace("#", "")
        .replace("$", "")
    ...
    for estimate_equipment_pricing in estimate_equipments_pricing:
        if estimate_equipment_pricing.equipment.service == service:
            if estimate_equipment_pricing.price_override:
                service_total = service_total + float(
...
def estimate_customer_adjustment_calculator(estimate_id):
    estimate = Estimate.objects.get(id=estimate_id)
    sub_total = estimate_sub_total_calculator(
        estimate_id
    ) + estimate_predemo_calculator(estimate_id)
    customer_adjustment = (
        estimate.customer.company.customer_adjustment_in_percentage * sub_total / 100
    )
...
def estimate_total_calculator(estimate_id):
    ...
    estimate_total = (
        float(estimate_sub_total) + float(pre_demo) + float(customer_adjustment)
    )
    estimate_total = round(estimate_total, 2)
    return estimate_total
...
def get_s3_file_url(key, pdf_type, version=None):
    ...
    if pdf_type == "estimate":
        return s3.get_bucket_object(
            key="media/pdfs/estimate/"
            + pdf_filename_generator(key, "E", version)
            + ".pdf"
        )
    if pdf_type == "quote":
        return s3.get_bucket_object(
            key="media/pdfs/quote/" + pdf_filename_generator(key, "Q") + ".pdf"
        )
    if pdf_type == "proposal":
        return s3.get_bucket_object(
            key="media/pdfs/proposal/" + pdf_filename_generator(key, "P") + ".pdf"
        )
    if pdf_type == "changeorder":
        return s3.get_bucket_object(
            key="media/pdfs/changeorder/" + changeorder_filename_generator(key) + ".pdf"
        )
    if pdf_type == "invoice":
        return s3.get_bucket_object(key="media/pdfs/invoice/" + key + ".pdf")
    if pdf_type == "account_summary":
        return s3.get_bucket_object(
            key="media/pdfs/accountsummary/AccountSummary-" + key + ".pdf"
        )
    if pdf_type == "submittal":
        return s3.get_bucket_object(
            key="media/pdfs/submittal/submittal-"
            + submittal_number_generator(key)
            + ".pdf"
        )
    if pdf_type == "report":
        return s3.get_bucket_object(
            key="media/pdfs/report/FINAL SHEET "
            + key.project.proposal.estimate.project.name.upper()
            + "-"
            + str(key.project.project_number)
            + ".pdf"
        )
    else:
