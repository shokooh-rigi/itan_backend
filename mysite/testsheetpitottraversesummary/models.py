...
class PitotTraverseSummaryEquipment(models.Model):
...
class PitotTraverseSummarySheetData(models.Model):
    data_type = models.PositiveSmallIntegerField(choices=DataTypeChoices.get_items(), default=1, null=False)
    pitot_traverse_summary_equipment = models.ForeignKey(PitotTraverseSummaryEquipment, on_delete=models.CASCADE, blank=False, null=False)
    ...
    def __str__(self):
        return str(self.sheet_field) + ' ' + str(self.pitot_traverse_summary_equipment)
