...
from .models import PitotTraverseSummaryEquipment, PitotTraverseSummarySheetData
        ...
        if form.is_valid():
            if request.POST.get("next"):
                ...
                for n in range(form.cleaned_data['equipment_quantity']):
                    pitot_traverse_summary_equipment = PitotTraverseSummaryEquipment(sheet=sheet, equipment_type=equipment_type)
...
def pitot_traverse_summary_sheet_equipment_list(request, sheet_id):
    ...
    pitot_traverse_summary_equipments = PitotTraverseSummaryEquipment.objects.filter(sheet=my_sheet).order_by('id')
    if project_name:
...
def pitot_traverse_summary_equipment_add(request, sheet_id):
    my_sheet = DataSheet.objects.get(id=sheet_id)
    pitot_traverse_summary_equipment = PitotTraverseSummaryEquipment(sheet=my_sheet)
    pitot_traverse_summary_equipment.save()
...
def get_pdf_parameters(sheet_id, is_report_pdf: bool):
    total_pdf_row = 22
    my_sheet = DataSheet.objects.get(id=sheet_id)
    pitot_traverse_summary_equipments = PitotTraverseSummaryEquipment.objects.filter(sheet=my_sheet, design_data_entry_completed=True).order_by('id')
    if is_report_pdf:
    ...
    for pitot_traverse_summary_equipment in pitot_traverse_summary_equipments:
        ...
        if is_report_pdf:
            pitottraversesummarysheetdatas = pitot_traverse_summary_equipment.pitottraversesummarysheetdata_set.all()
        else:
            pitottraversesummarysheetdatas = pitot_traverse_summary_equipment.pitottraversesummarysheetdata_set.filter(data_type=1)
...
def pitot_traverse_summary_design_data(request, equipment_id):
    equipment = get_object_or_404(PitotTraverseSummaryEquipment, id=equipment_id)
    ...
    if request.method == 'POST':
        ...
        if request.POST.get("next"):
            ...
            for design_field in design_fields:
                ...
                num_results = PitotTraverseSummarySheetData.objects.filter(data_type=1, pitot_traverse_summary_equipment=equipment,
                                                           sheet_field=design_field).count()

                if num_results > 0:
                    PitotTraverseSummarySheetData.objects.filter(data_type=1, pitot_traverse_summary_equipment=equipment,
                                                 sheet_field=design_field).update(value=new_value)
                else:
                    new_object = PitotTraverseSummarySheetData(data_type=1, pitot_traverse_summary_equipment=equipment,
    ...
    if request.method == 'POST':
        ...
        if request.POST.get("next"):
            ...
            for actual_field in actual_fields:
                ...
                num_results = PitotTraverseSummarySheetData.objects.filter(data_type=2, pitot_traverse_summary_equipment=equipment,
                                                           sheet_field=actual_field).count()

                if num_results > 0:
                    PitotTraverseSummarySheetData.objects.filter(data_type=2, pitot_traverse_summary_equipment=equipment,
                                                 sheet_field=actual_field).update(value=new_value)
                else:
                    new_object = PitotTraverseSummarySheetData(data_type=2, pitot_traverse_summary_equipment=equipment,
