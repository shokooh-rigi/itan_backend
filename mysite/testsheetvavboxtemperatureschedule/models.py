...
class VbtsEquipment(models.Model):
...
class VbtsSheetData(models.Model):
    data_type = models.PositiveSmallIntegerField(choices=DataTypeChoices.get_items(), default=1, null=False)
    vbts_equipment = models.ForeignKey(VbtsEquipment, on_delete=models.CASCADE, blank=False, null=False)
    ...
    def __str__(self):
        return str(self.sheet_field) + ' ' + str(self.vbts_equipment)
