    ...
    def post(self, request):
        ...
        **Response**:
        ...
        if form_serializer.is_valid():
            ...
            to_email = (
                form_serializer.validated_data["to_email"].replace(" ", "").split(",")
            )
            cc = (
                form_serializer.validated_data["cc"].replace(" ", "").split(",")
                if form_serializer.validated_data["cc"]
                else []
            )
            ...
            user_data = {
                "name": (
                    ...
                    if user.last_name
                    else "TAB Technologies, INC. Operator"
                ),
                ...
                "tel": (
                    user.profile.tel
                    if user.profile.tel
                    else LicenseInfo.objects.get(key="OwnerTel").value + " Office"
                ),
            }

            invoice_template = (
                ModulesToEmailTemplateRelation.objects.get(module=9).template.content
                if ModulesToEmailTemplateRelation.objects.filter(module=9).exists()
                else "No template defined."
            )
            footer_template = (
                ModulesToEmailTemplateRelation.objects.get(module=5).template.content
                if ModulesToEmailTemplateRelation.objects.filter(module=5).exists()
                else "No footer template defined."
            )

            # Prepare and send email
            try:
                footer_content = (
                    footer_template.replace("[user_name]", user_data["name"])
                ...
                msg = EmailMessage(
                    subject, message, settings.DEFAULT_FROM_EMAIL, to_email, cc=cc
                )
                msg.content_subtype = "html"
                msg.attach_file(
                    f"media/pdfs/accountsummary/AccountSummary-{email_id}.pdf"
                )
                msg.send()
                return Response(
                ...
                return Response(
                    {"error": "Invalid email header found."},
...
class AccountSummaryListView(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        serializer = AccountSummarySerializer(company)
...
class AccountSummaryCreateView(APIView):
    ...
    def post(self, request, customer_id=None):
        ...
        if serializer.is_valid():
            ...
            customer_invoices, total = AccountSummaryService.calculate_invoices_total(
                customer
            )

            if total == 0:
                return Response(
                    {"error": "This Customer has no remaining invoices to pay."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
