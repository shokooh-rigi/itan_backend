...
class BidListView(APIView):
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter(name="id", description="The ID of the bid", required=True, type=int),
        ],
        responses={
            200: BidSerializer,
            404: dict,
...
class BidListView(APIView):
    """
    API view to retrieve a list of bid with filtering and pagination options.

    - GET: Retrieves a list of bid based on optional query parameters for search, date range, and ordering.
    """
    ...
    def get(self, request) -> Response:
        """
        Retrieve a list of bid, optionally filtered by search term, date range, and ordering.

        Query Parameters:
            - search: Filter by project name or customer company name.
            - ordering: Order the results by a field (default is 'due_date').
            - fromDate: Start date for filtering bid by due date (mm/dd/yyyy).
            - toDate: End date for filtering bid by due date (mm/dd/yyyy).
...
class BidCreateView(APIView):
    """
    Create a new bid with uploaded files.
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                ...
                'due_date': openapi.Schema(type=openapi.TYPE_STRING, description='Due date for the bid'),
    ...
    def _get_temp_path():
        ...
        temp_path = os.path.join(
            os.path.abspath(os.path.dirname("__file__")), "media/uploads/bids"
        )
    ...
    def _save_bid_to_db(self, customer_id, project_id, due_date, zip_file_path):
        """
        Save the bid entry to the database and upload the zip to S3.
        """
