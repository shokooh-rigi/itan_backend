...
from .models import PrimaryHeatExchanger2Equipment, PrimaryHeatExchanger2SheetData
        ...
        if form.is_valid():
            if request.POST.get("next"):
                ...
                for n in range(form.cleaned_data['equipment_quantity']):
                    primary_heat_exchanger_2_equipment = PrimaryHeatExchanger2Equipment(sheet=sheet, equipment_type=equipment_type)
...
def primary_heat_exchanger_2_sheet_equipment_list(request, sheet_id):
    ...
    primary_heat_exchanger_2_equipments = PrimaryHeatExchanger2Equipment.objects.filter(sheet=my_sheet).order_by('id')
    if project_name:
...
def primary_heat_exchanger_2_equipment_add(request, sheet_id):
    my_sheet = DataSheet.objects.get(id=sheet_id)
    primary_heat_exchanger_2_equipment = PrimaryHeatExchanger2Equipment(sheet=my_sheet)
    primary_heat_exchanger_2_equipment.save()
...
def get_pdf_parameters(sheet_id, is_report_pdf: bool):
    total_pdf_row = 4
    my_sheet = DataSheet.objects.get(id=sheet_id)
    primary_heat_exchanger_2_equipments = PrimaryHeatExchanger2Equipment.objects.filter(sheet=my_sheet, design_data_entry_completed=True).order_by('id')
    if is_report_pdf:
    ...
    for primary_heat_exchanger_2_equipment in primary_heat_exchanger_2_equipments:
        ...
        if is_report_pdf:
            primary_heat_exchanger_2_sheetdatas = primary_heat_exchanger_2_equipment.primaryheatexchanger2sheetdata_set.all()
        else:
            primary_heat_exchanger_2_sheetdatas = primary_heat_exchanger_2_equipment.primaryheatexchanger2sheetdata_set.filter(data_type=1)
...
def primary_heat_exchanger_2_common_data(request, equipment_id):
    primary_heat_exchanger_2_equipment = get_object_or_404(PrimaryHeatExchanger2Equipment, id=equipment_id)
    this_equipment = None
    ...
    if request.method == 'POST':
        ...
        if request.POST.get("next"):
            ...
            for design_field in design_fields:
                ...
                num_results = PrimaryHeatExchanger2SheetData.objects.filter(data_type=1, primary_heat_exchanger_2_equipment=primary_heat_exchanger_2_equipment,
                                                           sheet_field=design_field).count()

                if num_results > 0:
                    PrimaryHeatExchanger2SheetData.objects.filter(data_type=1, primary_heat_exchanger_2_equipment=primary_heat_exchanger_2_equipment,
                                                 sheet_field=design_field).update(value=new_value)
                else:
                    new_object = PrimaryHeatExchanger2SheetData(data_type=1, primary_heat_exchanger_2_equipment=primary_heat_exchanger_2_equipment,
    ...
    if request.method == 'POST':
        ...
        if request.POST.get("next"):
            ...
            for actual_field in actual_fields:
                ...
                num_results = PrimaryHeatExchanger2SheetData.objects.filter(data_type=2, primary_heat_exchanger_2_equipment=primary_heat_exchanger_2_equipment,
                                                           sheet_field=actual_field).count()

                if num_results > 0:
                    PrimaryHeatExchanger2SheetData.objects.filter(data_type=2, primary_heat_exchanger_2_equipment=primary_heat_exchanger_2_equipment,
                                                 sheet_field=actual_field).update(value=new_value)
                else:
                    new_object = PrimaryHeatExchanger2SheetData(data_type=2, primary_heat_exchanger_2_equipment=primary_heat_exchanger_2_equipment,
